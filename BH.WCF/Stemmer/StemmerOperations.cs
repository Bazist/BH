...
namespace FTSearchWCF.Stemmers
{
    public abstract class StemmerOperations
    {
        ...
        public string StemContent(string content)
        {
            ...
            {
                if (i == words[count].Index)
                {
                    //insert stemmed word
                    string word = words[count].Word;
                    string stemWord = Stem(words[count].Word);
                ...
                //content = Regex.Replace(content, word, Stem(word), RegexOptions.IgnoreCase);
            }

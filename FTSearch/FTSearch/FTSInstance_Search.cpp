								 ...
								 uint32 maxPage)
{
	if (!pAllKeysAndValuesRAM)
	{
		...
		pAllKeysAndValuesRAM = HArrayFixPair::CreateArray(Info.CountWordsRAM + Info.CountWordsHDD, countKeySegments);
		pAllKeysAndValuesRAMCount = Info.CountWordsRAM + Info.CountWordsHDD;

		HArrayFixRAM* pTempHAWordsRAM = 0;

		//save haWordsHDD
		if (Info.CountWordsHDD)
		{
			...
			uint32 count = pTempHAWordsRAM->getKeysAndValuesByRange(pAllKeysAndValuesRAM,
																  ...
																  0,
																  0);
			haWordsHDD.open(haWordsHDD.HeaderSize * 4);
		}
		...
		if(Info.CountWordsRAM)
		{
			...
			//merge results
			uint32 currIndex = Info.CountWordsHDD;

			for (uint32 i = 0; i < count; i++)
			{
				if (pTempHAWordsRAM->getValueByKey(pKeysAndValuesRAM[i].Key) == 0)
				{
					pKeysAndValuesRAM[i].copyTo(pAllKeysAndValuesRAM[currIndex++], countKeySegments);
				}
			}
		...
		//HArrayFixPair::sortByValue(pKeysAndValuesHDD, Info.CountWordsHDD, true, countKeySegments);
	}
	...
	for(uint32 i=0; i < pAllKeysAndValuesRAMCount; i++)
	{
		...
		//DocumentsBlock* p = (DocumentsBlock*)pAllKeysAndValuesRAM[i].Value;

		//uint32 count1 = countTotalDocuments;
		//uint32 count2 = p->CountDocuments;

		HArrayVisitor::getWord(tempWord,
							   pAllKeysAndValuesRAM[i].Key,
							   Configuration.AutoStemmingOn);

		/*if(strcmp(word2, "купить") == 0)
		{
			word2[0] = word2[0];
		}*/
									  ...
									  uint32& sourceBuffLength)
{
	...
	return (double)(equals * 2) / (double)(countUsedDocNumbers + equals + notEquals);
